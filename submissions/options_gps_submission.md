...
3. **Exchange Layer (`exchange.py`)** — Fetches live quotes from Deribit (JSON-RPC 2.0) and Aevo (REST with HMAC-SHA256). Each exchange is implemented as a thin adapter behind a common interface, making the layer highly extensible — adding a new venue (e.g., Derive, OKX, Bybit) requires only a single adapter function with no changes to the pipeline or ranking logic. Computes per-leg price divergences between Synth fair value and exchange prices, enabling Market Line Shopping — each leg is routed to the cheapest venue automatically. The more exchanges connected, the wider the search for mispriced liquidity.
...
- **Probabilistic data enables automated line shopping.** Because Synth provides fair value pricing, the tool can objectively compare exchange quotes and route to the best venue per leg — something not possible with directional-only signals. The adapter-based exchange layer is designed so that plugging in additional venues immediately deepens this comparison without touching the core pipeline.
